(ns golem.canvas-plot
  (:use [golem.numeric :only [pi abs pow round avg log ; needed
;                            sin exp erfgamma digamma sigmoid
                            ]
...
(defn plot! [cvs xy-maps x-start x-end y-start y-end]
     (do (clear! cvs)
         (let [c (get-context cvs)
               width (.-clientWidth cvs)
               height (.-clientHeight cvs)
               x-scale-width 75
               y-scale-width 30]
...
#_(let [start (- pi)
        ...
        erng (create-range -2 0 100)
        nrng (normalize rng start end)]
    (plot! canvas [{:x-dat nrng
                    :y-dat (normalize (map #(.exp js/Math %) erng))
                    :type :cont
